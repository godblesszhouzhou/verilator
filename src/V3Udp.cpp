...
class UdpVisitor final : public VNVisitor {
    ...
    void visit(AstUdpTable* nodep) override {
        ...
        if (m_comboNodeStmtp) {
            ...
            if (!m_alwaysIfMapp.empty()) m_alwaysp = new AstAlways{fl, VAlwaysKwd::ALWAYS_LATCH, nullptr, nullptr};
            else m_alwaysp = new AstAlways{fl, VAlwaysKwd::ALWAYS, nullptr, nullptr};
    ...
    bool isEdgeTrig(std::string& valName) {
        ...
        if (valName.length() == 4) {
            ...
            if (valNum == "01" || valNum == "p" || valNum == "P") {
                valName = "r";
                return true;
            }
            if (valNum == "10" || valNum == "n" || valNum == "N") {
                valName = "f";
                return true;
            }
